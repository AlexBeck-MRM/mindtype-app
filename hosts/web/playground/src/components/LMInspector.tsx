...
const CodeBlock: React.FC<{ title: string; content: string | null | undefined }> = ({
  title,
  content,
}) => (
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div>
          ...
          <div>
            <b>Band:</b> {info.band ? `[${info.band.start}, ${info.band.end}]` : '—'}
          </div>
        </div>
        <div>
          <div>
            <b>Span length:</b> {info.span?.length ?? 0}
          </div>
          <div>
            <b>Recent chunks:</b> {(info.lastChunks || []).slice(-5).join(' | ') || '—'}
          </div>
        </div>
      </div>
