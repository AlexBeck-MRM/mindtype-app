...
function App() {
  ...
  // Collapsible right-side panel (persistent)
  const [panelOpen, setPanelOpen] = useState<boolean>(() => {
    try { return localStorage.getItem('mt-panel-open') !== 'false'; } catch { return true; }
  });
  ...
  // Persist panel state
  useEffect(() => {
    try { localStorage.setItem('mt-panel-open', String(panelOpen)); } catch {}
  }, [panelOpen]);

  // Keyboard shortcut: Cmd/Ctrl + /
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      const isToggle = (e.key === '/' || e.code === 'Slash') && (e.metaKey || e.ctrlKey);
      if (isToggle) {
        e.preventDefault();
        setPanelOpen((v) => !v);
      }
  ...
  const handleTextChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    // Forward to pipeline for real-time corrections
    try {
      pipelineRef.current?.ingest(nextValue, nextCaret);
    } catch {}
  ...
  return (
    <div className="App">
      ...
      <div style={{ 
        display: 'grid', 
          gridTemplateColumns: panelOpen ? '1fr 2fr 1fr' : '1fr 2fr',
        ...
        <div style={{ 
          ...
          {/* RIGHT PANEL - Controls (floating, collapsible) */}
        {panelOpen && (
          <div style={{ 
              position: 'fixed',
              right: '5vh',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <h3 style={{ margin: 0, fontSize: '0.95em', color: '#7ce0b8' }}>Controls</h3>
                <button onClick={() => setPanelOpen(false)} aria-label="Close controls (Cmd/Ctrl + /)" title="Close (Cmd/Ctrl + /)" style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.2)' }}>✕</button>
              </div>
            {/* Core Controls */}
        ...
        {!panelOpen && (
          <button
            ...
            aria-label="Open controls (Cmd/Ctrl + /)"
            title="Open controls (Cmd/Ctrl + /)"
            style={{
              position: 'fixed',
              right: '5vh',
              bottom: '5vh',
