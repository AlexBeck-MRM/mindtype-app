...
export function LMLab() {
  ...
  const adapter = useMemo(
    () =>
      createWorkerLMAdapter(
        () =>
          new Worker(new URL('../worker/lmWorker.ts', import.meta.url), {
            type: 'module',
          }),
      ),
    [],
  ...
  const [dynamicPresets, setDynamicPresets] = useState<
    Array<{ id: string; name: string; text: string; tone?: string }>
  >([]);
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (cleaned && sel.span && contextManager.validateProposal(cleaned, sel.span)) {
        ...
        console.log('[LMLab] Proposal rejected:', {
          original: sel.span,
          proposal: cleaned,
        });
      } else if (!cleaned) {
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
          {dynamicPresets.map((p) => (
            <button
              key={p.id}
              onClick={() => applyPreset(p.text, { tone: p.tone as Tone })}
            >
              {p.name}
            </button>
          ))}
        ...
        />
        <div
          style={{ marginTop: '0.5rem', display: 'flex', gap: 8, alignItems: 'center' }}
        >
          <button onClick={run} disabled={running} data-testid="lm-run">
            Run
          </button>
          {dynamicPresets.length === 0 && (
            <>
              <div style={{ fontSize: 12, opacity: 0.8 }}>Presets:</div>
              <button onClick={() => applyPreset('helloo thr weathfr has beenb hood')}>
                Typos
              </button>
              <button onClick={() => applyPreset('I has went to the store.')}>
                Grammar
              </button>
              <button
                onClick={() =>
                  applyPreset("We can't ship this, it's kinda bad.", {
                    tone: 'Professional',
                  })
                }
              >
        ...
        {error && (
          <div role="alert" style={{ marginTop: 8, color: '#f66', fontSize: 12 }}>
            {error}
          </div>
        )}
        <div
        ...
        >
          <div>
            ...
            <pre data-testid="lm-context-output" style={{ whiteSpace: 'pre-wrap' }}>
              {contextOut}
            </pre>
          </div>
          <div>
            <h3>Tone Output</h3>
            <pre data-testid="lm-tone-output" style={{ whiteSpace: 'pre-wrap' }}>
              {toneOut}
            </pre>
          </div>
        </div>
        ...
        <div style={{ marginTop: '0.5rem' }}>
          <label>
            ...
            <input
              ...
              step={0.01}
              value={tauTone}
              onChange={(e) => setTauTone(parseFloat(e.target.value))}
            />
          </label>
        </div>
        <div style={{ marginTop: '0.5rem' }}>
          <label>
            ...
            <input
              type="number"
              value={bandStart}
              onChange={(e) => setBandStart(parseInt(e.target.value || '0'))}
            />
          </label>
        </div>
        <div style={{ marginTop: '0.5rem' }}>
          <label>
            ...
            <input
              type="number"
              value={bandEnd}
              onChange={(e) => setBandEnd(parseInt(e.target.value || '0'))}
            />
          </label>
        </div>
