...
import { isSecureElement } from '../../../src/ui/securityDetection';

interface WasmCaretMonitor {
  flush(): void;
  on_snapshot(cb: (snaps: CaretSnapshot[]) => void): void;
  update(params: {
    kind: string;
    timestamp_ms: number;
    caret: number;
  ...
  get_stats?(): unknown;
  get_thresholds(): Record<string, number>;
  set_thresholds(thresholds: Record<string, number>): void;
}

interface WasmModule {
  default?: () => Promise<void>;
  WasmCaretMonitor?: new () => WasmCaretMonitor;
}
...
function readSelection(el: Element | null): {
  collapsed: boolean;
  start: number;
  end: number;
} {
...
function computeCaret(
  el: Element | null,
  sel: { start: number; end: number },
): { caret: number; text_len: number } {
...
async function createTSShim(root?: Document | HTMLElement): Promise<CaretShim> {
  ...
  const handler = (ev: Event) => {
    ...
    if (ev.type === 'beforeinput') {
      ...
      if (it && it.startsWith('delete')) {
        const now = Date.now();
        if (now - lastDeleteMs <= deleteWindowMs) deleteCountWindow += 1;
        else deleteCountWindow = 1;
        lastDeleteMs = now;
      }
      lastTyping = Date.now();
    }
    if (ev.type === 'keydown') {
      ...
      if (e.key.startsWith('Arrow')) {
        ...
        if (Math.abs(nowCaret - lastCaret) >= jumpThreshold)
          caretJumpUntil = Date.now() + 300;
  ...
  return {
    detach() {
      alive = false;
      types.forEach((t) => target.removeEventListener(t, handler, true));
    },
    onSnapshot(cb) {
      onSnaps = cb;
    },
...
export async function createCaretShim(
  root?: Document | HTMLElement,
): Promise<CaretShim | null> {
  if (typeof window === 'undefined' || typeof document === 'undefined') return null;
  let mod: WasmModule | null = null;
  try {
    // Dynamically import bundled wasm pkg via local dependency alias.
    mod = await import('@mindtype/core');
  ...
  let monitor: WasmCaretMonitor;
  try {
    if (!mod?.WasmCaretMonitor) {
      return createTSShim(root);
    }
  ...
  function handle(ev: Event) {
    ...
    const input = (ev as InputEvent).inputType;
