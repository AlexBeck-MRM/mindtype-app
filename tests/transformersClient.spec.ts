...
import { createTransformersAdapter, detectBackend } from '../src/lm/transformersClient';
...
describe('Transformers client', () => {
  it('streams chunks from runner, band-bounded', async () => {
    ...
    adapter.init?.();
    const out: string[] = [];
    for await (const c of streamWithLegacyParams(adapter, {
      text: 'abc teh def',
      caret: 11,
      band: { start: 4, end: 7 },
    })) {
  ...
  it('supports abort()', async () => {
    ...
    adapter.init?.();
    const it = streamWithLegacyParams(adapter, {
      text: 'hello world',
      caret: 5,
      band: { start: 0, end: 5 },
  ...
  it('enforces single-flight + cooldown', async () => {
    ...
    const it1 = streamWithLegacyParams(adapter, {
      text: 'X one two',
      caret: 9,
      band: { start: 2, end: 9 },
    })[Symbol.asyncIterator]();
    ...
    const it2 = streamWithLegacyParams(adapter, {
      text: 'Y one two',
      caret: 9,
      band: { start: 2, end: 9 },
    })[Symbol.asyncIterator]();
  ...
  it('respects backend-specific cooldown (webgpu faster than cpu)', async () => {
    ...
    for await (const _ of streamWithLegacyParams(cpuAdapter, {
      text: 'ab',
      caret: 2,
      band: { start: 0, end: 2 },
    })) {
    ...
    for await (const _ of streamWithLegacyParams(gpuAdapter, {
      text: 'ab',
      caret: 2,
      band: { start: 0, end: 2 },
    })) {
  ...
  it('increases cooldown on wasm when threads are not available', async () => {
    ...
    for await (const _ of streamWithLegacyParams(adapter, {
      text: 'ab',
      caret: 2,
      band: { start: 0, end: 2 },
    })) {
