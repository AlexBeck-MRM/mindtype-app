...
import { renderMechanicalSwap } from '../src/ui/swapRenderer';
...
describe('swapRenderer events', () => {
  ...
  it('emits legacy highlight events with original text', () => {
    const highlights: Array<{ originalText?: string; text?: string }> = [];
    (
      globalThis as unknown as {
        addEventListener: (t: string, l: (e: MTEvent) => void) => void;
      }
    ).addEventListener('mindtype:highlight', (e: MTEvent) =>
      highlights.push(e.detail as { originalText?: string; text?: string }),
    );

    renderMechanicalSwap({ start: 5, end: 7, text: 'bar', originalText: 'foo' });
    expect(highlights).toHaveLength(1);
    expect(highlights[0]).toMatchObject({ text: 'bar', originalText: 'foo' });
  });
});
