...
import { streamMerge } from '../src/lm/mergePolicy';
import type { LMAdapter, LMStreamParams } from '../src/lm/types';

const TEST_TOKEN_CAPS = { webgpu: 4096, wasm: 2048, cpu: 1024 };
...
function makeAdapter(chunks: string[]): LMAdapter {
  return {
    init: () => ({ backend: 'cpu', maxContextTokens: 512, tokenCaps: TEST_TOKEN_CAPS }),
    abort: () => {},
    async *stream(_params: LMStreamParams): AsyncIterable<string> {
