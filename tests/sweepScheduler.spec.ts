...
// Mock correction wave to observe LM-only pipeline calls
...
describe('SweepScheduler', () => {
  ...
  it('drops events and clears timers in secure/IME contexts', async () => {
    ...
    const mockLM = { stream: async function* () {} } as any;
    const scheduler = createSweepScheduler(monitor, security, () => mockLM);
    scheduler.start();
    ...
    expect(runCorrectionWave).not.toHaveBeenCalled();
    // v0.6: backfillConsistency removed
