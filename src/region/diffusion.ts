/*╔══════════════════════════════════════════════════════════╗
  ...
  ║                                                            ║
  ╚══════════════════════════════════════════════════════════╝
  • WHAT ▸ Active region policy (render vs context ranges); Never modify at or after the caret; Single undo per sweep (atomic)
  • WHY  ▸ CONTRACT-ACTIVE-REGION, REQ-CARET-SAFETY, REQ-UNDO-GROUPING
...
import { replaceRange } from '../safety/diff';
import type { LMAdapter } from '../lm/types';
import type { ActiveRegionPolicy } from '../region/policy';
    ...
    try {
      ...
      for await (const ev of streamMerge({
        ...
        if (ev.type === 'diff' && ev.diff) {
          try {
            const previous = state.text.slice(ev.diff.start, ev.diff.end);
