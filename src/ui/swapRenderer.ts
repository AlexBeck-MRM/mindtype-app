...
export function renderMechanicalSwap(
  range: { start: number; end: number; text: string; originalText?: string },
  config: SwapConfig = {},
) {
  ...
  if (g.dispatchEvent && g.CustomEvent) {
    ...
    g.dispatchEvent(event);
    dispatchLegacyHighlight(range, site.originalText);
  }
...
export function emitSwap(range: {
  start: number;
  end: number;
  text: string;
  originalText?: string;
...
export function renderHighlight(range: {
  start: number;
  end: number;
  text?: string;
  originalText?: string;
...
function dispatchLegacyHighlight(
  range: { start: number; end: number; text: string; originalText?: string },
  originalText?: string,
) {
  const g = globalThis as unknown as MinimalGlobal;
  ...
  const event = new g.CustomEvent('mindtype:highlight', {
    detail: { start: range.start, end: range.end, text: range.text, originalText },
  });
  g.dispatchEvent(event);
}
