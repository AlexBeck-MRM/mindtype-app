...
export function createMockStreamLMAdapter(): LMAdapter {
  ...
  return {
    ...
    async *stream(params: LMStreamParams): AsyncIterable<string> {
      ...
      const { text, activeRegion } = normalized;
      const bandText = text.slice(activeRegion.start, activeRegion.end);
      ...
      function applyTone(s: string, tone: 'None' | 'Casual' | 'Professional'): string {
        if (tone === 'Professional')
          return s.replace(/^(\w)/, (_, c) => 'Consequently, ' + c);
        if (tone === 'Casual')
          return s.replace(/^(\w)/, (_, c) => 'Hey, ' + c.toLowerCase());
        return s;
      }
