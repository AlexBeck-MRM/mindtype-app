...
export interface LMStreamParams {
  ...
  /**
   * @deprecated Use `activeRegion`. Provided for backward compatibility.
   */
...
export const normalizeLMStreamParams = (params: LMStreamParams): LMStreamParams => {
  const region = params.activeRegion ?? params.band;
  if (!region) {
    throw new Error('LMStreamParams.ActiveRegion is required');
  }
  return {
    ...params,
    activeRegion: region,
    band: params.band ?? region,
  };
};
